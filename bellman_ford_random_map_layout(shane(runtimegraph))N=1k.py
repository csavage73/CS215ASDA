...
def generate_random_map(num_nodes=2000, width=4000, height=2600, extra_edges=300, min_dist=70, seed=42):
...
def draw_graph_with_path(G, positions, path, source, target, cost):
    ...
    # Draw weights only for small graphs (too cluttered at scale)
    if not large_graph:
        for u, v, data in G.edges(data=True):
            x1, y1 = positions[u]
            x2, y2 = positions[v]
...
def measure_time(func, *args, trials=3):
    times = []
...
if __name__ == "__main__":
    ...
    for n in sizes:
        ...
        # Measure ONLY Bellman-Ford (not graph generation)
        t = measure_time(bellman_ford, graph_dict, SOURCE)

        nodes.append(n)
        times.append(t)

        print(f"Nodes: {n}, Time: {t:.6f} seconds")
    ...
    plt.title("Bellman-Ford Time Complexity")
    plt.grid(True)
    plt.show()