...
if __name__ == "__main__":
    ...
    while i < rounds:
        ...
        #print("map created for test: ", i)
        fix_negative_cycles(G)
        graph_dict = nx_to_adj_dict(G)
        #pprint(graph_dict)
        
        
        #print("Strating algorithm")
        tracemalloc.reset_peak() #resets peak for looping

        start_time = time.perf_counter()  #start timer

        distances, predecessors = bellman_ford(graph_dict, SOURCE)  #algorithm running
        result = reconstruct_path(predecessors, SOURCE, TARGET)
        shortest_path, path = result if result else (None, [])

        end_time = time.perf_counter()    #end timer
        current, peak = tracemalloc.get_traced_memory() #get memory peak
        try:
            ...
            groundTest = None  # negative cycle present, cannot verify

        times.append(end_time - start_time)
        storage.append(peak)
        #print("printing shortest path", shortest_path)
        i += 1
    tracemalloc.stop()
