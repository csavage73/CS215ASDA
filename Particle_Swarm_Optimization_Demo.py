...
W  = 0.7   # inertia (probability of random mutation)
...
SOURCE = "N01"
TARGET = f"N{NUM_NODES}"  # change to total nodes
searchSpace = []
...
def random_path(G, source, target, max_steps=10000):
    visited = {source}
    path = [source]
    current = source
...
def initialize_swarm(G, source, target, num_particles, searchSpace):
    swarm = []
...
def run_pso(G, source=SOURCE, target=TARGET):
    ...
    swarm = initialize_swarm(G, source, target, NUM_PARTICLES, searchSpace)
...
if __name__ == "__main__":
    ...
    while i < rounds:
        ...
        #run the PSO
        pso_path, pso_cost, history, sol_path = run_pso(G, SOURCE, TARGET)
