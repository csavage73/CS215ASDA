...
def generate_random_map(num_nodes=100, width=200, height=130, extra_edges=15, min_dist=7, seed=random.randint(0,100)):  # change num_nodes default here if not passing it from __main__
    ...
    for dist, u, v in possible_edges:
        if added >= extra_edges:
            break
        percent_value = 23 #PERCENT FOR COVERAGE**
        chance = random.randint(0, 99)
        if chance < percent_value:
            congested = random.randint(5,10)
            G.add_edge(u, v, weight=congested)
...
SOURCE = "N01"
TARGET = "N200"  # change to total nodes
...
def random_path(G, source, target, max_steps=20000, bfs_dist=None):  # PATH FIX - added bfs_dist parameter
    """BFS-guided random walk: biases toward neighbors closer to target."""
    if bfs_dist is None:  # PATH FIX - compute BFS distances from target if not provided
        bfs_dist = nx.single_source_shortest_path_length(G, target)  # PATH FIX - hop counts from every node to target
    visited = {source}
    path = [source]
    current = source
    for _ in range(max_steps):
        ...
        if not neighbors:
            ...
            neighbors.sort(key=lambda n: bfs_dist.get(n, float("inf")))  
            top = max(1, len(neighbors) // 2)  
            nxt = random.choice(neighbors[:top]) 
            path.append(nxt)
            visited.add(nxt)
            current = nxt
    return None
...
class Particle:
    def __init__(self, G, source, target, seed_path=None):
        path = None
        if seed_path is not None:
            # Try a few quick mutations of the seed; fall back to seed itself
            for _ in range(5):
                candidate = mutate(G, seed_path)
...
def draw_pso_result(G, positions, pso_path, dijkstra_path):
...
if __name__ == "__main__":
    ...
    G, positions = generate_random_map(
        num_nodes=200,   # CHANGE NODE COUNT HERE — also update TARGET above to match (e.g. num_nodes=50 -> TARGET="N50")
        width=850,        # increase width/height if nodes are too cramped after raising num_nodes
        height=550,
        extra_edges=200,  # more nodes = more edges needed to keep the graph well-connected
    ...
    pso_elapsed = time.perf_counter() - pso_start
    print(f"PSO runtime       : {pso_elapsed:.3f}s")

    dijkstra_path = nx.dijkstra_path(G, SOURCE, TARGET, weight="weight")
    draw_pso_result(G, positions, pso_path, dijkstra_path)